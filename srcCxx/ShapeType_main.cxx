...
const char *DEFAULT_KEY_AGREEMENT_ALG   = "ECDH";
        ...
        const char *partition, float livelinessPeriod, bool enable_logging)
{
    ...
    DomainParticipant *participant = ShapeTypeConfigurator::create_participant(domain_id, use_security, governance_file, permissions_file, kee_agreement_algorithm, enable_logging);
...
void print_usage( const char * name )
{
  ...
  printf( "    [-kagree <keyAgreementAlgorithm>] :  default: '%s'\n", DEFAULT_KEY_AGREEMENT_ALG)
...
int main(int argc, char *argv[])
{
    ...
    const char *kee_agreement_algorithm = NULL;
    ...
    for (int i=1; i<argc; ++i) {
        ...
        else if ( strcmp(argv[i], "-kagree") == 0 ) {
            if ( ++i == argc) {
                fprintf(stderr, "Error: missing <keyAgreementAlgorithm> after \"-kagree\"\n");
                return -1;
            }
            kee_agreement_algorithm = argv[i];
        }
    ...
    if ( use_security ) {
      ...
      if ( kee_agreement_algorithm == NULL ){
        kee_agreement_algorithm  = DEFAULT_KEY_AGREEMENT_ALG;
        fprintf(stderr, "Info: \"-kagree\" unspecified. Default to \"%s\"\n",
                kee_agreement_algorithm);
      }
    }
    ...
    return run(domain_id, use_security, published_topic, subscribed_topic, color_name,
               governance_file, permissions_file, kee_agreement_algorithm,
